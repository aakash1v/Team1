from flask import Flask, redirect, url_for, render_template, flash, jsonify, request, send_file, Response
...
from routers.team1 import login_bp, login_manager, track_login, track_logout
...
def new_home():
    if not current_user.is_authenticated:
        return redirect(url_for("auth.login"))
    # Query all projects with their product owners
...
def charts():       
    try:
        ...
        if not team_performance_data:
            return render_template('charts.html', performance_chart_url="")
...
def generate_scheduled_report():
    try:
        # Generate the PDF
        pdf = generate_pdf()
        # Save the PDF to a predetermined location
        with open(f'reports/automated_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.pdf', 'wb') as f:
            f.write(pdf.get_data())
    except Exception as e:
        print(f"An error occurred while generating the scheduled report: {e}")


# Schedule automated reports
schedule.every().day.at("08:41").do(generate_scheduled_report)
schedule.every().monday.at("00:00").do(generate_scheduled_report)
schedule.every().day.at("00:00").do(lambda: generate_scheduled_report()
