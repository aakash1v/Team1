...
import os
from models import ProjectDetails, ProductOwner, UserStories, ScrumMasters, Users, ProjectUsers, SprintCalender
...
def submit_project_data():
    try:
        ...
        if not data.get('product_owner_id') or not isinstance(data['product_owner_id'], int):
        ...
        try:
            ...
            revised_end_date = (
                ...
                if data.get('revised_end_date') else None
            )
        except ValueError:
        ...
        if data.get('status') not in ["Active", "Completed", "On Hold","Cancelled"]:
            flash("Invalid status. Must be one of: Active, Completed, or Pending.", "error")
            return redirect(request.referrer)
        ...
        for i, sprint in enumerate(data['sprints'], start=1):
            ...
            if sprint_start_date >= sprint_end_date:
                flash(f"Sprint {i} start date cannot be greater than or equal to the end date.", "error")
                return redirect(request.referrer)

            if not sprint.get('scrum_master_id') or not isinstance(sprint['scrum_master_id'], int):
                flash(f"Valid Scrum Master ID is required for Sprint {i}.", "error")
                return redirect(request.referrer)
            scrum_master = ScrumMasters.query.get(sprint['scrum_master_id'])
            if not scrum_master:
                flash(f"Scrum Master with ID {sprint['scrum_master_id']} not found.", "error")
                return redirect(request.referrer)
            ...
            for j, story in enumerate(sprint['user_stories'], start=1):
                if not story.get('description') or len(story['description'].strip()) == 0:
                    flash(f"Description is required for User Story {j} in Sprint {i}.", "error")
                    return redirect(request.referrer)
                if not story.get('planned_sprint') or not isinstance(story['planned_sprint'], int):
                    flash(f"Valid Planned Sprint is required for User Story {j} in Sprint {i}.", "error")
                    return redirect(request.referrer)
                if not story.get('actual_sprint') or not isinstance(story['actual_sprint'], int):
                    flash(f"Valid Actual Sprint is required for User Story {j} in Sprint {i}.", "error")
                    return redirect(request.referrer)
                if story.get('moscow') not in ["Must Have", "Should Have", "Could Have", "Won't Have"]:
                    flash(f"Invalid MoSCoW priority for User Story {j} in Sprint {i}.", "error")
                    return redirect(request.referrer)
                if not story.get('assignee'):
                    flash(f"Assignee is required for User Story {j} in Sprint {i}.", "error")
                    return redirect(request.referrer)
...
def edit_project(project_id):
    ...
    # Handle POST request (optional, for saving updates)
        ...
        try:
            ...
            try:
                ...
                revised_end_date = datetime.strptime(request.form['revised_end_date'], '%Y-%m-%d').date()
            ...
            for index, sprint in enumerate(project.sprints):
                ...
                try:
                    sprint_start_date = datetime.strptime(request.form[f'sprint_start_date_{index+1}'], '%Y-%m-%d').date()
                    sprint_end_date = datetime.strptime(request.form[f'sprint_end_date_{index+1}'], '%Y-%m-%d').date()

                    if sprint_end_date < sprint_start_date:
                        flash(f"End Date cannot be earlier than Start Date for Sprint {index+1}.", "error")
                        return redirect(request.referrer)
                except ValueError:
                    flash(f"Invalid date format for Sprint {index+1}. Use YYYY-MM-DD.", "error")
                    return redirect(request.referrer)

                velocity = request.form.get(f'sprint_velocity_{index+1}')
                if not velocity.isdigit() or int(velocity) <= 0:
                    flash(f"Velocity for Sprint {index+1} must be a positive number.", "error")
                    return redirect(request.referrer)
                ...
                for user_story_index, user_story in enumerate(sprint.user_stories):
                    user_story_description = request.form.get(f'story_desc_{index+1}_{user_story_index}', '').strip()
                    if not user_story_description:
                        flash(f"Description is required for User Story {user_story_index+1} in Sprint {index+1}.", "error")
                        return redirect(request.referrer)

                    planned_sprint = request.form.get(f'planned_sprint_{index+1}_{user_story_index}')
                    actual_sprint = request.form.get(f'actual_sprint_{index+1}_{user_story_index}')

                    if not planned_sprint.isdigit() or not actual_sprint.isdigit():
                        flash(f"Planned and Actual Sprint for User Story {user_story_index+1} in Sprint {index+1} must be numbers.", "error")
                        return redirect(request.referrer)

                    story_point = request.form.get(f'story_points_{index+1}_{user_story_index}')
                    if not story_point.isdigit() or int(story_point) <= 0:
                        flash(f"Story Point for User Story {user_story_index+1} in Sprint {index+1} must be a positive number.", "error")
                        return redirect(request.referrer)

                    moscow = request.form.get(f'moscow_{index+1}_{user_story_index}', '').strip()
                    if moscow not in ['must-have', 'should-have', 'could-have', 'won\'t-have']:
                        flash(f"Invalid MOSCOW value for User Story {user_story_index+1} in Sprint {index+1}.", "error")
                        return redirect(request.referrer)

                    assignee = request.form.get(f'assignee_{index+1}_{user_story_index}', '').strip()
                    if not assignee:
                        flash(f"Assignee is required for User Story {user_story_index+1} in Sprint {index+1}.", "error")
                        return redirect(request.referrer)

                    status = request.form.get(f'status_{index+1}_{user_story_index}', '').strip()
                    if status not in ['not-started', 'in-progress', 'completed']:
                        flash(f"Invalid status for User Story {user_story_index+1} in Sprint {index+1}.", "error")
                        return redirect(request.referrer)
    ...
    return render_template('edit_project.html', project=project, scrum_masters=scrum_masters)
